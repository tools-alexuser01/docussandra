...
import static com.strategicgains.docussandra.bucketmanagement.ApplicationCF.ENTITY_INDEX;
import static com.strategicgains.docussandra.bucketmanagement.ApplicationCF.ENTITY_INDEX_ENTRIES;
import static com.strategicgains.docussandra.bucketmanagement.DbConstants.DICTIONARY_CONNECTED_ENTITIES;
...
import it.unimi.dsi.fastutil.shorts.ShortIterators.EmptyIterator;
...
public class NewRelationManagerImpl implements NewRelationManager {

    private static final Logger logger = LoggerFactory.getLogger(NewRelationManagerImpl.class);

    //private EntityManagerImpl em;
    private IndexCassandraDao cass;
    private UUID applicationId;
    private UUID headEntity;
    private IndexBucketLocator indexBucketLocator;

    public NewRelationManagerImpl() {
    }

    public NewRelationManagerImpl init( /*EntityManagerImpl em,*/IndexCassandraDao cass, UUID applicationId,
            UUID headEntity, IndexBucketLocator indexBucketLocator) {

        //Assert.notNull( em, "Entity manager cannot be null" );
        if (cass == null) {
            throw new IllegalArgumentException("Cassandra service cannot be null");
        }
        if (applicationId == null) {
            throw new IllegalArgumentException("Application Id cannot be null");
        }
        if (headEntity == null) {
            throw new IllegalArgumentException("Head entity uuid cannot be null");
        }
        if (indexBucketLocator == null) {
            throw new IllegalArgumentException("Index bucket locator cannot be null");
        }
    ...
    private NewRelationManagerImpl getRelationManager(UUID headEntity) {
        NewRelationManagerImpl rmi = new NewRelationManagerImpl();
        rmi.init(/*em,*/ cass, applicationId, headEntity, indexBucketLocator);
        return rmi;
    }

//    /**
//     * side effect: converts headEntity into an Entity if it is an UUID!
//     */
//    private Document getHeadEntity() throws Exception {
//        Document entity = null;
//        if (headEntity instanceof Document) {
//            entity = (Document) headEntity;
//        } else {
...
//    public IndexUpdate batchUpdateCollectionIndex( IndexUpdate indexUpdate, UUID owner, String collectionName )
//            throws Exception {
//
//        logger.debug( "batchUpdateCollectionIndex" );
//
//        Document indexedEntity = indexUpdate.getEntity();
//
...
//
//
//    public Map<UUID, Set<String>> getContainingCollections() throws Exception {
//        Map<UUID, Set<String>> results = new LinkedHashMap<UUID, Set<String>>();
//
...
//            }
//        }
//        UUID applicationRef = new SimpleEntityRef( TYPE_APPLICATION, applicationId );
...
//    public void batchCreateCollectionMembership( Mutator<ByteBuffer> batch, UUID ownerRef, String collectionName,
//                                                 UUID itemRef, UUID membershipRef, UUID timestampUuid )
//            throws Exception {
//
...
//     * @throws Exception the exception
//     */
//    public Mutator<ByteBuffer> batchAddToCollection( Mutator<ByteBuffer> batch, String collectionName, Document entity,
...
//                                                      String collectionName, Document entity, UUID timestampUuid )
//            throws Exception {
//
//        long timestamp = getTimestampInMicros( timestampUuid );
//
...
//                    for ( UUID ownerId : ownerIds ) {
//                        UUID owner = new SimpleEntityRef( ownerType, ownerId );
...
//                                                          Document entity, UUID timestampUuid ) throws Exception {
...
//    public Mutator<ByteBuffer> batchRemoveFromCollection( Mutator<ByteBuffer> batch, String collectionName,
//                                                          Document entity, boolean force, UUID timestampUuid )
//            throws Exception {
//
//        long timestamp = getTimestampInMicros( timestampUuid );
//
...
//     */
//    private PagingResultsIterator getReversedConnectionsIterator( UUID targetEntity, String connectionType )
//            throws Exception {
...
//        long timestamp = getTimestampInMicros( timestampUuid );
//
//        Document connectedEntity = em.get( connection.getConnectedEntityId() );
//
...
//
//
//    public IndexUpdate batchStartIndexUpdate( List<BoundStatement> batch, Document entity, String entryName,
...
//        if ( !skipRead ) {
//
//            List<Column<ByteBuffer, ByteBuffer>> entries = null;
//
...
//
//            // Delete all matching entries from entry list
//            for ( Column<ByteBuffer, ByteBuffer> entry : entries ) {
...
//        if (containers != null) {
//
//            Map<UUID, Set<String>> containerEntities = null;
//            if (noRead) {
//                containerEntities = new LinkedHashMap<UUID, Set<String>>();
//                UUID applicationRef = new SimpleEntityRef(TYPE_APPLICATION, applicationId);
...
//            }
//
//            for (UUID containerEntity : containerEntities.keySet()) {
...
//    public void batchUpdateSetIndexes(List<BoundStatement> batch, String setName, Object elementValue,
...
//
//        if (containers != null) {
//            Map<UUID, Set<String>> containerEntities = getContainingCollections();
//            for (UUID containerEntity : containerEntities.keySet()) {
//                if (containerEntity.getType().equals(TYPE_APPLICATION) && Schema
//                        .isAssociatedEntityType(entity.getType())) {
//                    logger.debug("Extended properties for {} not indexed by application", entity.getType());
...
//    public boolean isCollectionMember( String collectionName, UUID entity ) throws Exception {
//
//        Keyspace ko = cass.getApplicationKeyspace( applicationId );
//
...
//    public boolean isConnectionMember( String connectionName, UUID entity ) throws Exception {
//        Keyspace ko = cass.getApplicationKeyspace( applicationId );
//
...
//
//        for ( UUID owner : containerEntities.keySet() ) {
...
//    public Entity addToCollection( String collectionName, UUID itemRef ) throws Exception {
//
//        Document itemEntity = em.get( itemRef );
//
//        if ( itemEntity == null ) {
//            return null;
//        }
...
//    public Entity addToCollections( List<UUID> owners, String collectionName ) throws Exception {
//
//        Document itemEntity = getHeadEntity();
//
...
//
//        Document itemEntity = em.create( itemType, properties );
//
//        if ( itemEntity != null ) {
//            UUID timestampUuid = newTimeUUID();
//            Mutator<ByteBuffer> batch = CountingMutator.createFlushingMutator( cass.getApplicationKeyspace( applicationId ), Serializers.be );
...
//    public void removeFromCollection( String collectionName, UUID itemRef ) throws Exception {
//
//        if ( headEntity.getUuid().equals( applicationId ) ) {
...
//            if ( collectionName.equals( COLLECTION_ROLES ) ) {
//                String path = ( String ) ( ( Document ) itemRef ).getMetadata( "path" );
...
//        Map<UUID, Set<String>> containers = getContainingCollections();
//        if ( containers != null ) {
//            for ( Entry<UUID, Set<String>> container : containers.entrySet() ) {
...
//    public void copyRelationships( String srcRelationName, UUID dstEntityRef, String dstRelationName )
//            throws Exception {
//
...
//                List<UUID> refs = results.getRefs();
//                for ( UUID ref : refs ) {
...
//    public ConnectionRef createConnection( String connectionType, UUID connectedEntityRef ) throws Exception {
//
//        headEntity = em.validate( headEntity );
...
//    public ConnectionRef createConnection( String pairedConnectionType, UUID pairedEntity, String connectionType,
//                                           UUID connectedEntityRef ) throws Exception {
//
...
//    public ConnectionRef connectionRef( String connectionType, UUID connectedEntityRef ) throws Exception {
//
...
//    public ConnectionRef connectionRef( String pairedConnectionType, UUID pairedEntity, String connectionType,
//                                        UUID connectedEntityRef ) throws Exception {
//
...
//     */
//    private Results getConnectedEntities( UUID sourceEntity, String connectionType, String connectedEntityType,
...
//     */
//	private Results getConnectingEntities(UUID targetEntity,
...
//     * @param resultsLevel The results level to return
//     */
//    private Results getConnectingEntities( UUID targetEntity, String connectionType, String connectedEntityType,
//                                           Level resultsLevel ) throws Exception {
...
//            UUID ref = em.getAlias( headEntity.getUuid(), collection.getType(), nameIdentifierNode.getName() );
//
//            if ( ref == null ) {
...
//
//        /**
//         * True if we should search from source->target edges. False if we
...
//         * @param outgoing The direction to search. True if we should search
//         * from source->target edges. False if we should search from
//         * target<-source edges
//         */
