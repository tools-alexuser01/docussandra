...
public class ConnectedIndexScanner implements IndexScanner {
    ...
    private final IndexCassandraDao cass;
     ...
     */
    private LinkedHashSet<Column<ByteBuffer, ByteBuffer>> lastResults;
    ...
    public ConnectedIndexScanner(IndexCassandraDao cass, String dictionaryType, UUID applicationId, UUID entityId,
            Iterator<String> connectionTypes, ByteBuffer start, boolean reversed, int pageSize,
            boolean skipFirst) {

        if (entityId == null) {
            throw new IllegalArgumentException("Entity id for row key construction must be specified when searching graph indexes");
        }
     ...
     * collection. Load the next page. Return false if nothing was loaded, true
     * otherwise
     */
    public boolean load() throws Exception {
        ...
        lastResults = new LinkedHashSet<Column<ByteBuffer, ByteBuffer>>();

        //cleanup columns for later logic
        //pointer to the first col we load
        Column<ByteBuffer, ByteBuffer> first = null;

        //pointer to the last column we load
        Column<ByteBuffer, ByteBuffer> last = null;
        ...
        while (currentConnectionType != null) {
            ...
            Object key = NewCassandraPersistanceUtils.key(entityId, dictionaryType, currentConnectionType);

            List<Column<ByteBuffer, ByteBuffer>> results = null;//TODO: udeyoje make this work
     ...
     */
    @Override
    public Iterator<Set<Column<ByteBuffer, ByteBuffer>>> iterator() {
        return this;
    }
     ...
     */
    @Override
    //@Metered( group = "core", name = "IndexBucketScanner_load" ) --//i don't think this is needed: udeyoje
    public Set<Column<ByteBuffer, ByteBuffer>> next() {
        Set<Column<ByteBuffer, ByteBuffer>> returnVal = lastResults;
