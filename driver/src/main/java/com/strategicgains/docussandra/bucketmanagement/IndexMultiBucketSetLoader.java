...
public class IndexMultiBucketSetLoader {
    ...
    /**
     * Loads and sorts columns from each bucket in memory. This will return a
     * contiguous set of columns as if they'd been read from a single row
     */
    public static TreeSet<Column<ByteBuffer, ByteBuffer>> load(IndexCassandraDao cass, ApplicationCF columnFamily,
            UUID applicationId, List<Object> rowKeys, Object start,
            Object finish, int resultSize, boolean reversed)
            throws Exception {
        Map<ByteBuffer, List<Column<ByteBuffer, ByteBuffer>>> results
                //TODO: udeyoje: fix
                = null;//cass.multiGetColumns(cass.getApplicationKeyspace(applicationId), columnFamily, rowKeys, start,
        ...
        TreeSet<Column<ByteBuffer, ByteBuffer>> resultsTree
                = new TreeSet<Column<ByteBuffer, ByteBuffer>>(new Comparator<Column<ByteBuffer, ByteBuffer>>() {

                    @Override
                    public int compare(Column<ByteBuffer, ByteBuffer> first,
                            Column<ByteBuffer, ByteBuffer> second) {

                        return comparator.compare(first.getName(), second.getName());
                    }
                });

        for (List<Column<ByteBuffer, ByteBuffer>> cols : results.values()) {

            for (Column<ByteBuffer, ByteBuffer> col : cols) {
                resultsTree.add(col);
